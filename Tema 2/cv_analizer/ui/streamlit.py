...
from prompts.cv_evaluator import evaluar_candidato
...
def mostrar_area_resultados():
    ...
    if st.session_state.get('analizar', False):
        ...
        if archivo_cv is None:
            st.error("⚠ Por favor sube un archivo PDF con el currículum")
            return
            
        if not descripcion_puesto:
            st.error("⚠ Por favor proporciona una descripción detallada del puesto")
            return
...
def mostrar_resultados(resultado: AnalisisCV):
    ...
    with col1:
        st.info(f"**👨‍⚕️ Nombre:** {resultado.nombre_candidato}")
        st.info(f"**🕓 Experiencia:** {resultado.experiencia_años} años")
